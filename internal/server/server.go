...
func New(db *store.DB, limits Limits) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{"tier": s.limits.Tier, "upgrade_url": "https://stockyard.dev/permit/"})})
	return s
}
...
func (s *Server) createPermits(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 { if s.db.CountPermits() >= s.limits.MaxItems { we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/permit/"); return } }
	var e store.Permits
...
func (s *Server) delPermits(w http.ResponseWriter, r *http.Request) {
	s.db.DeletePermits(r.PathValue("id"))
