...
func (s *Server) activateLicense(w http.ResponseWriter, r *http.Request) {
	...
	if err := PersistLicense(s.dataDir, key); err != nil {
		log.Printf("permit: license persist failed: %v", err)
	...
	s.limMu.Unlock()
	log.Printf("permit: license activated via dashboard, persisted to %s/%s, tier=%s", s.dataDir, licenseFilename, newTier)
...
func (s *Server) listPermits(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"permits": oe(s.db.SearchPermits(q, filters))})
		return
	}
...
func (s *Server) createPermits(w http.ResponseWriter, r *http.Request) {
	...
	if e.PermitType == "" {
		we(w, 400, "permit_type required")
		return
	}
	if e.HolderName == "" {
		we(w, 400, "holder_name required")
		return
	}
...
func (s *Server) updatePermits(w http.ResponseWriter, r *http.Request) {
	...
	if patch.PermitType == "" {
		patch.PermitType = existing.PermitType
	}
	if patch.HolderName == "" {
		patch.HolderName = existing.HolderName
	}
	if patch.HolderEmail == "" {
		patch.HolderEmail = existing.HolderEmail
	}
	if patch.PermitNumber == "" {
		patch.PermitNumber = existing.PermitNumber
	}
	if patch.IssuedDate == "" {
		patch.IssuedDate = existing.IssuedDate
	}
	if patch.ExpiryDate == "" {
		patch.ExpiryDate = existing.ExpiryDate
	}
	if patch.IssuingAuthority == "" {
		patch.IssuingAuthority = existing.IssuingAuthority
	}
	if patch.Status == "" {
...
func (s *Server) delPermits(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	s.db.DeletePermits(id)
	s.db.DeleteExtras("permits", id)
