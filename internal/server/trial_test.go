...
// Tests for the additive trial-gate middleware on permit. Booking
...
//   7. End-to-end: tier=="none" → POST /api/permits returns 402
//   8. End-to-end: tier=="paid" → POST /api/permits returns 200
//   9. End-to-end: tier=="none" → GET /api/permits still works (read-through)
...
// permit's existing limits.go doesn't expose a hook for that.

func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "permit-trial-test-*")
	if err != nil {
...
func newTestServer(t *testing.T, tier string) *Server {
	t.Helper()
	dir, err := os.MkdirTemp("", "permit-trial-srv-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(dir) })
...
func TestShouldBlockWrite_NoneTier(t *testing.T) {
	...
	for _, c := range cases {
		req := httptest.NewRequest(c.method, "/api/permits", nil)
...
func TestShouldBlockWrite_ExpiredTier(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/permits", nil)
	if !s.shouldBlockWrite(req) {
		t.Error("expired tier should block POST writes")
	}
	req = httptest.NewRequest("GET", "/api/permits", nil)
	if s.shouldBlockWrite(req) {
...
func TestShouldBlockWrite_PaidTier(t *testing.T) {
	...
	for _, m := range []string{"POST", "PUT", "DELETE", "PATCH", "GET"} {
		req := httptest.NewRequest(m, "/api/permits", nil)
		if s.shouldBlockWrite(req) {
...
func TestEndToEnd_NoneTierBlocksWrite(t *testing.T) {
	s := newTestServer(t, "none")
	body := bytes.NewBufferString(`{"permit_type":"Building","holder_name":"Alice"}`)
	req := httptest.NewRequest("POST", "/api/permits", body)
...
func TestPersistLicense_WritesFile(t *testing.T) {
	dir, err := os.MkdirTemp("", "permit-persist-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
...
func TestLoadLicenseFromDisk_RoundTrip(t *testing.T) {
	dir, err := os.MkdirTemp("", "permit-load-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
...
func TestLoadLicenseFromDisk_MissingReturnsEmpty(t *testing.T) {
	dir, err := os.MkdirTemp("", "permit-missing-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
